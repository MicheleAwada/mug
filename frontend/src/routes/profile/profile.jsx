                ...
                {(!isMobile() || (isMobile() && isIndex)) ? <section className={"h-full bg-gray-50 " + (isMobile() ? "w-full" :"w-64")}>
                    <ul className="bg-gray-100 border-4 border-gray-200 border-t-0 border-l-0 w-full h-full">
                        <li className={"h-20 w-full border-b-2 border-gray-200 " + (selectedItem === 'details' ? "bg-gray-300 border-gray-400" : "")}>
                            <a href="/profile/details/" className={"flex justify-center items-center h-full w-full text-gray-600 " + (selectedItem === "details" ? "text-gray-950" : "")}>Account Details</a>
                        </li>
                        <li className={"h-20 w-full border-b-2 border-gray-200 " + (selectedItem === 'security' ? "bg-gray-300 border-gray-400" : "")}>
                            <a href="/profile/security/" className={"flex justify-center items-center h-full w-full text-gray-600 " + (selectedItem === "security" ? "text-gray-950" : "")}>Security</a>
                        </li>
                        <li className={"h-20 w-full border-b-2 border-gray-200 " + (selectedItem === 'delete-account' ? "bg-gray-300 border-gray-400" : "")}>
                            <a href="/profile/delete-account/" className={"flex justify-center items-center h-full w-full text-gray-600 " + (selectedItem === "delete-account" ? "text-gray-950" : "")}>Delete Account</a>
                        </li>
                    </ul>
                </section> : <></>}
                {!isIndex && 
                <div className="py-4 px-8 w-full">
                    <a href="/profile/" className="w-12 h-12 sm:w-14 sm:h-14 md:w-14 md:h-14 lg:w-12 lg:h-12 bg-gray-400 rounded-full flex justify-center items-center"><IoArrowBackOutline className="w-12 h-12" color="white" alt="back" /></a>
